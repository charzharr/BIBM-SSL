...
class DenseuNet(BaseModel):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #  decoder
        self.deco_conv1 = nn.Conv2d(num_features, 768, kernel_size=3, padding=1, bias=False)
...
def denseunet121(pretrained=False, **kwargs):
    ...
    model = DenseuNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    print(f"\tDenseUNet 121 initialized ({model.param_counts[0]} params).")
    if pretrained:
...
def denseunet169(pretrained=False, **kwargs):
    ...
    model = DenseuNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 32, 32),
                     **kwargs)
    print(f"\tDenseUNet 169 initialized ({model.param_counts[0]} params).")
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
        # They are also in the checkpoints in model_urls. This pattern is used
...
def denseunet201(pretrained=False, **kwargs):
    ...
    model = DenseuNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 48, 32),
                     **kwargs)
    print(f"\tDenseUNet 201 initialized ({model.param_counts[0]} params).")
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
        # They are also in the checkpoints in model_urls. This pattern is used
...
def denseunet161(pretrained=False, **kwargs):
    ...
    model = DenseuNet(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24),
                     **kwargs)
    print(f"\tDenseUNet 161 initialized ({model.param_counts[0]} params).")
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
        # They are also in the checkpoints in model_urls. This pattern is used
...
def get_model(name, pretrained=False, num_classes=1):
    if '121' in name:
        model = denseunet121(pretrained=pretrained, num_classes=num_classes)
    elif '161' in name:
        model = denseunet161(pretrained=pretrained, num_classes=num_classes)
    elif '169' in name:
        model = denseunet169(pretrained=pretrained, num_classes=num_classes)
    elif '201' in name:
        model = denseunet201(pretrained=pretrained, num_classes=num_classes)
    else:
        raise ValueError('denseunet model: {name} is not supported.')
    return model
